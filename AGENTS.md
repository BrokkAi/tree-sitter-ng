...
- **Null Strictness**: The project aims to be null strict. Use `@Nullable` only when a value can genuinely be null, and enforce `@NonNull` (the JSpecify default for `@NullMarked` packages) elsewhere. Avoid allowing `null` for core domain concepts (e.g., a `TSTree` should always have a valid `TSLanguage`).
